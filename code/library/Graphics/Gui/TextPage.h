...
namespace GRAPHICS::GUI
{
    ...
    /// A page encompasses text that may be displayed in the text box at a given time.
    /// If more text is supplied to the text box than can fit on a single page, then
    /// the text will be split up across multiple pages.
    class TextPage
    {
    public:
        // CONSTANTS.
        /// The number of characters of padding per line of text in the page.
        /// In order to prevent the characters on each line from running up against the border of the text box,
        /// one character on each side of each line is reserved to provide padding.
        static constexpr unsigned int ONE_CHARACTER_OF_PADDING_ON_EACH_SIDE_OF_LINE = 2;
        /// The time that must elapse for a single character to be displayed in the page.
        /// The displayed character count is time-based to ensure a a consistent speed in how fast
        /// text is displayed along with making sure it doesn't get displayed too quickly.
        static constexpr float ELAPSED_TIME_BETWEEN_CHARACTERS_IN_SECONDS = 0.03f;
        /// The scale ratio for text in the text box.
        static constexpr float TEXT_SCALE_RATIO = 1.0f;

        // CONSTRUCTION.
        explicit TextPage(const unsigned int width_in_pixels, const unsigned int height_in_pixels, const std::shared_ptr<Font>& font);
        ...
        /// The maximum amount of width (in pixels) available in the text box for text.
        /// This accounts for borders of the text box, decreasing the overall available width.
        const unsigned int MaxAvailableWidthForTextInPixels;
